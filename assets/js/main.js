...
window.onload = function () {
    ...
    // ON SUBMIT FORM QR
    $(function () {
		$("#form_qr").on("submit", function (e) {
            $(".downloads-block__right").html('<img src="assets/img/background/cogs.gif">')
            e.preventDefault();
            $.ajax({
                type: "POST",
                ...
                success: function (data){
                    if(data.success){
                        const qr = data.payload
                        $(".downloads-block__right").html('<img src="'+qr+'" width="300px"><p class="text-success-qr">'+data.message+'</p>')
                    }else{
                        $(".downloads-block__right").html('<p class="text-qr">'+data.message+'</p>')
                    }
                },
                error: function(data){
